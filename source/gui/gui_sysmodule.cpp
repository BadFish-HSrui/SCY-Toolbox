...
GuiSysmodule::GuiSysmodule() : Gui() {
    ...
    std::ifstream configFile("sdmc:/switch/SCYToolbox/config.json");
    json configJson;
            ...
            if (kdown & HidNpadButton_A) {
                ...
                if (this->m_runningSysmodules.find(sysmodule.first) != this->m_runningSysmodules.end()) {
                    if (!sysmodule.second.requiresReboot) {
                        ...
                        (new MessageBox("该系统插件修改需要重启生效。\n 重启switch以应用修改。", MessageBox::OKAY))->show();
                    }
...
void GuiSysmodule::draw() {
    ...
    if (anyModulesPresent)
        Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 150, currTheme.textColor, "选择需要运行的系统插件(后台服务)。\n 由于内存限制，可能无法同时启动所有服务。", ALIGNED_CENTER);
    else
        Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 550, currTheme.textColor, "您目前没有安装任何受支持的系统插件。\n 如需使用此功能，请安装受支持的系统插件(NSP格式)。", ALIGNED_CENTER);

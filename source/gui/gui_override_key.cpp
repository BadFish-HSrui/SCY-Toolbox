...
GuiOverrideKey::GuiOverrideKey() : Gui() {
    ...
    keySelectButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
        if (m_inputBlocked)
            gui->drawTextAligned(font20, x + 190, y + 100, currTheme.textColor, "请按任意键...", ALIGNED_CENTER);
    ...
    comboPressedButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
        gui->drawTextAligned(font20, x + 180, y + 50, currTheme.selectedColor, m_override.key.overrideByDefault ? "不按下" : "按下", ALIGNED_RIGHT);
        gui->drawTextAligned(font20, x + 180, y + 50, currTheme.textColor, "时启用", ALIGNED_LEFT);
    };
    ...
    switch (g_keyType) {
        case OverrideKeyType::Any_App_Override: {
            ...
            anyTitleIconButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                gui->drawTextAligned(font24, x, y - 60, currTheme.textColor, "覆盖应用对象:", ALIGNED_LEFT);
            ...
            overrideEnabledButton->drawAction = [&](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                gui->drawTextAligned(font20, x + 180, y + 50, m_overrideAnyApp ? currTheme.selectedColor : currTheme.unselectedColor, m_overrideAnyApp ? "开启" : "关闭", ALIGNED_CENTER);
            };
            ...
            appIconButton->drawAction = [&, title{DumpTitle(m_override.programID)}](Gui *gui, u16 x, u16 y, bool *isActivated) {
                ...
                if (title.get() != nullptr && title->application_id != 0) {
                    ...
                    if (appletName == nullptr) {
                        ...
                        gui->drawTextAligned(font20, Gui::g_framebuffer_width / 2, y + 350, currTheme.activatedColor, "通过小程序打开HBmenu可能无法为自制软件提供足够内存。\n 建议从游戏中打开HBmenu以获得完整内存访问权限。", ALIGNED_CENTER);
                    }
